...
public class ActionExecutor
{
    ...
    private static readonly string[] DestructivePatterns =
    {
        ...
        "format ", "diskpart", "shutdown", "git reset --hard",
        "git clean -fd", "git push --force", "git push -f",
        "> nul", "dd if=", ":(){ :|:& };:"
    };
    ...
    CancellationToken ct)
    {
        ...
        if (IsDestructive(command) && !UI.Confirm($"Comando potencialmente destrutivo: '{command}'. Executar?"))
    ...
    CancellationToken ct)
    {
        ...
        if (IsDestructive(command) && !UI.Confirm($"PowerShell potencialmente destrutivo: '{command}'. Executar?"))
            return "Operação cancelada.";
    ...
    private readonly the_bolotas.Memory.MemoryManager _memory;

    public ActionExecutor(ClaudeClient client)
    {
        _client = client;
        _memory = new the_bolotas.Memory.MemoryManager(PastaAtual);
    }

    private string Remember(JsonElement el)
    {
        var file = el.GetProperty("file").GetString() ?? "";
        var content = el.GetProperty("content").GetString() ?? "";
        return _memory.Append(file, content);
    }

    public string GetMemoryFormatted() => _memory.LoadFormatted();
    private async Task<string> FixBuildAsync(CancellationToken ct)
    {
        ...
        var prompt = FixCodePrompt.Build(
            buildResult,
            code,
            error.File
        );

        var aiResponse = await _client.AskAsync(prompt, ct);

        return await ExecuteAsync(aiResponse, ct);
    }
    private string UndoLast()
    {
        var snapshotDir = Path.Combine(PastaAtual, ".snapshots");
        ...
        var originalName = fileName.Substring(16); // yyyyMMdd_HHmmss_ = 16 chars
        var targetPath = Path.Combine(PastaAtual, originalName);

        File.Copy(latest, targetPath, true);

        return $"Restaurado: {targetPath}";
    }
    private void CreateSnapshot(string filePath)
{
    ...
    var backupName =
        $"{timestamp}_{Path.GetFileName(filePath)}";

    var backupPath =
        Path.Combine(snapshotDir, backupName);
